...
/*
 * HAL_GPIO_TogglePin(), GPIOA, GPIO_PIN_5 사용을 위해 포함
 * 보통 main.h만으로도 되지만, gpio.h를 명시적으로 포함해두는 것이 안전하다.
 */
#include "gpio.h"
 ...
 * 1000ms = 1초
 */
#define CAN_SEND_PERIOD_MS 1000
...
/*
 * main.c에 정의해둔 CAN 송신 함수
 *
 * main.c 쪽에 아래 함수가 있어야 한다.
 *
 * void CAN_Send_Message(uint32_t stdId)
 * {
 *   ...
 * }
 */
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN StartDefaultTask */
	  ...
	  GPIO_PinState prevButton0 = GPIO_PIN_SET;
	  GPIO_PinState prevButton1 = GPIO_PIN_SET;

	  GPIO_PinState nowButton0;
	  GPIO_PinState nowButton1;

	  for(;;)
	  {
	    nowButton0 = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
	    nowButton1 = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1);

	    /*
	     * PA0 버튼이 눌리는 순간
	     * Pull-up 방식이므로 SET -> RESET으로 바뀌는 순간이 버튼 누름
	     */
	    if (prevButton0 == GPIO_PIN_SET && nowButton0 == GPIO_PIN_RESET)
	    {
	      CAN_Send_Message(0x101);

	      HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);

	      osDelay(50); // 간단한 채터링 방지
	    }

	    /*
	     * PA1 버튼이 눌리는 순간
	     */
	    if (prevButton1 == GPIO_PIN_SET && nowButton1 == GPIO_PIN_RESET)
	    {
	      CAN_Send_Message(0x102);
	    ...
	    prevButton0 = nowButton0;
	    prevButton1 = nowButton1;
...
void StartTask02(void *argument)
{
  ...
  /*
   * 현재 CAN 테스트에서는 사용하지 않는 Task.
   * 나중에 컨베이어 모터 제어용으로 사용 가능.
   */
