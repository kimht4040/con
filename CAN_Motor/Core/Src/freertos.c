...
extern volatile uint8_t g_motorRun;
...
void StepMotor_Write(uint8_t in1, uint8_t in2, uint8_t in3, uint8_t in4);
...
//	  }
  ...
  for (;;)
  {
    if (g_motorRun == 0)
    {
      StepMotor_Write(0, 0, 0, 0);
      osDelay(10);
      continue;
    }

    StepMotor_Write(1, 0, 0, 0);
    osDelay(3);
